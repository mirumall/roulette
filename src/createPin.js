      ...
      if (wheel.rotationTween) {
        ...
        if (this.pinClickCount > 28) {
          if (wheel === this.wheel2) {
            ...
            const targetSlice = secondWheelOptions.slices.find(
              (slice) => slice.text === "WINNER"
            );
            const targetStartAngle = secondWheelOptions.slices.reduce(
              (acc, slice) => {
            ...
            const additionalAngle =
              (360 + currentAngle - sliceCenterAngle) % 360;

            // 목표 각도로 애니메이션
            const finalAngle =
              currentAngle + direction * (720 + additionalAngle); // 720도는 최소 2회전 추가
            ...
            const targetSlice = gameOptions.slices.find(
              (slice) => slice.text === "GAME OVER"
            );
          ...
          // 랜덤 동작
          const randomFinalAngle = Phaser.Math.Between(1080, 1800);
